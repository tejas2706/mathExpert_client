                ...
                classesAndExams.map((eachClass, index)=>{
                    return (
                        <div className="contentAccordion__eachAccordion">
                            <Accordion className={classes.contentAccordian} expanded={ expanded === eachClass.id } onChange={handleChange(eachClass.id)}>
                                <AccordionSummary
                                    expandIcon={<ExpandMoreIcon style={{color:"black"}} />}
                                    aria-controls="panel1bh-content"
                                    id="panel1bh-header"
                                    >
                                        <Typography className={classes.contentHeading}>Class {eachClass.class}</Typography>
                                </AccordionSummary>
                                <AccordionDetails>
                                    <Typography className={classes.contentTypo}>
                                        <div className="content__exams">
                                            {
                                                eachClass.examIds.map((eachExam)=>{
                                                    return(
